...
| `live-clock.js` | Algorithmic | the live-edge truth table + slow-before-flush playbackRate regulator that recovers a starved H.264 player without ever flushing the decoder more than once per 4s (task 151, 2026-08-10 — "the picture never goes blank, and when it stops it starts again by itself"). PURE, so its gate drives it whole against a realistic drift ramp; loads before `render.js`, which runs it — [about](__about/live-clock.md) · [flow](__flow/live-clock.md) |
