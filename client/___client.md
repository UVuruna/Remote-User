...
| `zoom-crop.js` | Standard | WHAT the zoom asks the PC to encode — the layout FLOOR a crop may never widen past, and the gesture SETTLE that keeps one blink per finished pinch (owner report 2026-08-14, T76: "why is downscaling done even when the picture is zoomed"). PURE, so its gate runs it whole; loads before `render.js`, which measures the canvas and runs it — [about](__about/zoom-crop.md) |
