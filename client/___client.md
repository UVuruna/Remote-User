...
| `cube.js` | Standard | THE CUBE'S OWN MOTION — the corner-view face order, idle spin and momentum-burst decay, as an independent `createCube(el)` handle. Extracted from `loading.js` 2026-08-17 when the update card needed the SAME cube spinning as a small badge (priority C: never write the same function twice). PURE-DOM (no globals beyond the element it is handed), so a node test can drive it with a bare fake element; loads before both `loading.js` and `update-banner.js` — [about](__about/cube.md) |
...
| `loading.js` | Algorithmic | the FULL-SCREEN overlay and the SETTLE watcher — the animation lasts as long as the WORK does, never until the server merely answers (split from layouts.js 2026-08-07; its motion metric moved to `settle-motion.js` 2026-08-11; the cube's own motion moved to `cube.js` 2026-08-17, delegated to via one `createCube()` handle) — [about](__about/loading.md) · [flow](__flow/loading.md) |
...
| `lang-groups.js` | Algorithmic | LANGUAGE FIRST, VARIANT SECOND — the grouping BOTH language lists read (Settings → Voice and the dictation card), after the owner reported both as one flat pile 2026-08-13. Decides what makes two locales the SAME (language + RESOLVED script + region, so sr-RS and sr-Latn-RS are one row and two choices) and when an engine's voice variant is a name rather than machinery. PURE, so its gate runs it whole; loads before both cards — [about](__about/lang-groups.md) · [flow](__flow/lang-groups.md) |
...
| `row-tap.js` | Standard | THE ONE ROW ACTIVATOR — a row acts on the LIFTED finger and never prevents the browser's default, so a finger landing on a row may still scroll the list it sits in (owner report 2026-08-15: the notification-voice card's language list; the same defect task 227b had fixed inside `layout-create.js` alone, which is exactly why four other panels still carried it). Asks `hold-gesture.js`'s `pressVerdict`, loads after it and before every panel that builds a list — [about](__about/row-tap.md) |
...
| `zoom-crop.js` | Standard | WHAT the zoom reports to the PC — the layout FLOOR the reported rect may never widen past, and the gesture SETTLE that keeps one blink per finished pinch; the server turns the report into a resolution STEP, never a narrower crop (round 3, corrected same day — round 2's crop-per-pan was condemned live) (owner report 2026-08-14, T76: "why is downscaling done even when the picture is zoomed"). PURE, so its gate runs it whole; loads before `render.js`, which measures the canvas and runs it — [about](__about/zoom-crop.md) |
...
| `page-version.js` | Standard | THE PAGE MUST NOT OUTLIVE ITS OWN PROTOCOL (T94, owner report 2026-08-14): the PC updates itself while this document stays loaded, so the page remembers the FIRST `config`'s `app_version` and reloads itself on ANY different one — measured live, a v0.0.205 page against the v0.0.209 server erased every pinch within ~0.5 s. PURE, so its gate runs the decision whole; loads before `connection.js`, which owns the reload — [about](__about/page-version.md) |
| `update-banner.js` + `update-banner.css` | Standard | the in-app APK update offer — version compare, five card states (offer / permission / downloading / installing / failed), and the spinning cube BADGE (`cube.js`) with a REAL determinate progress fill once the shell reports bytes (rewritten 2026-08-17: the shell now downloads the APK itself instead of handing off to Android's opaque DownloadManager; an older shell without `Android.updateInApp` still gets the original honest INDETERMINATE bar, task 207). The CSS split off `style.css` the same round it crossed 1,000 lines (THE STRUCTURE LAW), the `window-offer.js`/`.css` pairing pattern; the JS split off `controls.js` the round it crossed 1,000 lines — loads right after `cube.js` — [about](__about/update-banner.md) |
...
| `panels.js` | Standard | Settings overlays: the Sets picker and the command chooser (split from controls.js 2026-08-05; the dictation card left it 2026-08-13 at the 1,000-line wall) — loads 9th — [about](__about/panels.md) |
| `dictation-card.js` | Standard | WHAT THIS DEVICE HEARS — the dictation language card: language rows grouped by language, one written sample per language heard in a voice of that language, the mute-beeps switch. Split from `panels.js` 2026-08-13 BY RESPONSIBILITY (panels.js = which controls ride; this = the device in his hand); also owns the grouped-list rows `notify.js` borrows — loads after `panels.js`, before `notify.js` — [about](__about/dictation-card.md) |
...
| `ledger-panel.js` | Standard | the Session Ledger card (T111) — Claude Code's own task list for the focused layout's project, asked for with `ledger_state {}` on open and drawn immediately either way (an older PC never answers); one row per task, expand for its description/question/evidence, an answer types back through the existing `paste_text {focus:"claude"}` path — loads after `claude-panels.js` — [about](__about/ledger-panel.md) |
| `ledger-panel.css` | Standard | the ledger card's own styling — the state dot, the row anatomy and the expanded body; everything else is `panels.css`'s `.sets-*` vocabulary — documented with `ledger-panel.js` in [about](__about/ledger-panel.md) |
...
| `layout-new.js` | Standard | THE NEW SOURCE — what the PC can OPEN (a row an app already holds is dimmed and untappable, his report 2026-08-13) and, from inside a layout, what THAT app can DO first (T29). Split off `layout-create.js` 2026-08-13 by responsibility: that file is the wizard, this one is about the PC's own programs — [about](__about/layout-new.md) |
