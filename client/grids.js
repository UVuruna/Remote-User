...
// the choice is made by LOOKING, never by reading "3-left".
function gridSketch(grid, orient) {
  ...
  const rects = cells.map(([x, y, w, h]) =>
    `<rect x="${x * 15 + 1}" y="${y * 15 + 1}" width="${w * 15 - 2}" height="${h * 15 - 2}" rx="2"/>`).join("");
  return `<svg class="lay-grid-ico" viewBox="0 0 30 30" fill="currentColor">${rects}</svg>`;
}
