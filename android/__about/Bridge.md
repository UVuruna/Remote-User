...
| Battery | `batteryStats()` — T80d, 2026-08-14. What THIS device's battery is doing, so every handset answers for its own hardware rather than being predicted from the owner's. A **new** method beside `netStats` for the standing reason (the page is served by the PC, the shell installed separately, so changed arity stops resolving). Two rules live inside it: a property the device refuses (`Integer.MIN_VALUE`, or the flat `0` every stubbed implementation returns) is **omitted from the JSON entirely** — a zero would read on the desktop as "this app costs nothing" — and the draw's **sign is never trusted**, since `BATTERY_PROPERTY_CURRENT_NOW` is inverted on a known share of OEMs, so the magnitude travels and the direction comes from `isCharging`. See [Traffic Battery](../../server/gui/__about/traffic_battery.md) for the words it becomes. |
