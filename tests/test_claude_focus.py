...
5. THE SERVER REALLY WIRES IT. A pure function nobody calls is a feature that
...
class FakeInjector:
    ...
    def press_chord(self, chord: str) -> None:
        self.ops.append(("chord", chord))

    def press_key(self, name: str) -> None:
        self.ops.append(("key", name))
...
class Stage:
    ...
    def __init__(self, process="code.exe", busy=False):
    ...
    def __exit__(self, *_exc):
        (content.clipboard, content.FOCUS_STEP_DELAY,
         content.PASTE_ENTER_DELAY) = self._saved
        return False
