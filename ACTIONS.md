...
| **Claude** | Usage · Model · **Thinking** · Menu | **Mode** · New chat · Rewind · Context · Resume (**Stop and Agents DELETED 2026-08-15** on the owner's ruling: Stop was Esc, which the Input set already has; Agents typed `/agents`, which the VS Code extension answers only with "Continue in Terminal" — useless from the phone. Compact MOVED to Claude Tools 2026-08-11; **Focus DELETED 2026-08-13** — the owner: a button whose whole job is to click into a text field is not needed, especially with Tab and `next_input` already doing it. It was also wired to `ctrl+esc`, which Windows reserves for the Start menu, so it most likely opened Start. Gated in `tests/test_actions_migration.py` so it cannot return.) |
...
- **Built-in action** — `{ "action": "<name>" }`, where `<name>` is:
  ...
  - `notices` — open the **Notices** card: WHEN this device listens for the PC. Two states the owner said kept being confused, named apart — *only while the app is open in the background* (the DEFAULT, and what 0.0.127 shipped: swiping Vibe Coder out of recents stops the channel and the PC holds notices for up to 30 minutes) and *always, even after I close the app*. A LOCKED phone with the app still running keeps its notices under BOTH choices — locking is not closing. Per device, through the prefs bridge, read by the shell's own service; the card also states the limit that a phone REBOOT stops the service until the app is opened once. In the POOL of **Settings** since 2026-08-12.
