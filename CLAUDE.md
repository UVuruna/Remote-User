...
- **Mouse modes** are modifier buttons held while a finger moves on the screen: `right` (tap → right click), `drag` (left button held), `scroll` (wheel, with momentum), `hover` (move cursor only, no click — for triggering hover UI). Default tap with no mode = left click.
