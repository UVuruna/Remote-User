...
| `ledger_hook.py` | Standard | the Claude Code `UserPromptSubmit`/`Stop` hooks that keep a project's session ledger honest — creates the file, stamps the turn, refuses a `Stop` when the ledger went unchanged or ungrammatical (T111) — [about](__about/ledger_hook.md) |
