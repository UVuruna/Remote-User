...
class TrafficWindow(QDialog):
    ...
    def _maybe_start_history(self, kind: str, since: float | None) -> None:
        ...
        # TIME zoom, never `is_zoomed()` — a rate-axis zoom changes no row of
        # the file, and narrowing the read for it is what let the clock drag
        # the read's own bounds forward every tick (T110; see
        # `ViewRange.is_time_zoomed`).
        if view.is_time_zoomed():
