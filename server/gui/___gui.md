...
| `traffic_battery.py` | Algorithmic | what this app costs the phone’s BATTERY while it runs, in words — a device that will not say SAYS so, never a blank and never a zero — [about](__about/traffic_battery.md) · [flow](__flow/traffic_battery.md) |
| `traffic_chart.py` | Algorithmic | the Traffic chart widget — the picture, the hover card (now naming device + stream), and the ZOOM: a drag rectangle visible while drawn, wheel, − / + / Reset — split off traffic_window.py 2026-08-15 — [about](__about/traffic_chart.md) · [flow](__flow/traffic_chart.md) |
| `traffic_spans.py` | Standard | which stretch of the recording a span NAME means, and how a zoomed view keys its read — both pure, so a gate can assert "Today" is local midnight without building a window; split off traffic_window.py 2026-08-18 — [about](__about/traffic_spans.md) |
| `traffic_legend.py` | Standard | one legend/status swatch, DRAWN with QPainter — never a font glyph, and its colour is a CALLABLE so a theme flip follows it; split off traffic_window.py 2026-08-18 — [about](__about/traffic_legend.md) |
| `traffic_zoom.py` | Algorithmic | the chart's TIME WINDOW as pure arithmetic — full span vs view, zoom around an anchor, the drawn rectangle clamped and floored, pixels ↔ seconds — [about](__about/traffic_zoom.md) · [flow](__flow/traffic_zoom.md) |
...
| `main_window_server.py` | Standard | the main window's power over the server it wraps — start / stop / restart / quit, off-thread with the buttons gated, and the exit funnel that releases every window we ever raised; a MIXIN, split off main_window.py 2026-08-18 — [about](__about/main_window_server.md) |
| `main_window_updates.py` | Standard | the in-app update as a state machine — the FIFTEEN-MINUTE check (once-per-start cost the owner a day), the download with its named failure reason, and the handover this PC does not come back from on its own; a MIXIN, split off main_window.py 2026-08-18 — [about](__about/main_window_updates.md) |
