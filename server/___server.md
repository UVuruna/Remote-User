...
| `layout_popup.py` | Algorithmic | WHOSE window just appeared, and where it belongs — a dialog of a member, a member's own new window or a process a member started is the layout's WORK, so it is brought into the layout's region (or opened full screen when it cannot fit) instead of being refused into a place the phone cannot reach; every stranger goes back to the fence untouched (owner eruption 2026-08-11, task 202) — [about](__about/layout_popup.md) · [flow](__flow/layout_popup.md) |
| `recents.py` | Standard | WHAT THIS PC CAN OPEN, so a layout can be made from a window that is not open yet — Explorer's Quick Access folders, VS Code's own cached Open Recent menu, and honestly nothing but New window / Incognito for Chrome; the chosen entry is launched and the window that WAS NOT STANDING BEFORE is handed to the creation flow (owner 2026-08-09, task 184) — [about](__about/recents.md) |
