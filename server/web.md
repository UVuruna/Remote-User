...
## Purpose
The FastAPI application: serves the client page and static files, authenticates the WebSocket, streams video out (H.264 or JPEG), streams the cursor position, and routes input messages to the injector. API docs endpoints are disabled — nothing is exposed beyond `/`, `/static`, `/ws`, `/upload`, `/ping`, `/app.apk`.

HTTP: `GET /` — the client page, EXCEPT for plain Android browsers: when the User-Agent says Android, lacks the APK WebView's `RemoteUserApp` marker, and the APK exists, they get `install.html` — the full-screen install funnel (owner rule: a phone browser never shows a half-working client; Install → Open the app → the `intent://pair?url=…` link hands the tokened URL to the app, which pairs itself). `GET /favicon.ico` (the logo — otherwise every fresh load logs a 404), `GET /ping` (auth-free 204 — reachability probe for the phone's in-page Tailscale wizard AND the Android shell's start-up address resolver; reveals nothing but "server exists"), `GET /app.apk` (the APK for the funnel's Install button), `GET /static/*` (client assets), `POST /upload?token=…` (phone → PC image: decodes the upload, puts it in the Windows clipboard via [Clipboard](clipboard.md) and **injects Ctrl+V** — the image lands in the focused box by itself, picking it was the whole gesture; token-gated). Upload decoding is Pillow-first with the HEIF opener registered — phone cameras default to **HEIC**, which OpenCV cannot read, and Pillow also applies the **EXIF orientation** (cv2.imdecode ignores it and portrait photos would paste rotated); OpenCV stays as the fallback decoder, and an undecodable upload logs its magic bytes.
