...
HTTP: `GET /` (client page), `GET /favicon.ico` (the logo — otherwise every fresh load logs a 404), `GET /ping` (auth-free 204 — reachability probe for the phone's in-page Tailscale wizard AND the Android shell's start-up address resolver; reveals nothing but "server exists"), `GET /static/*` (client assets), `POST /upload?token=…` (phone → PC image: decodes the upload, puts it in the Windows clipboard via [Clipboard](clipboard.md) and **injects Ctrl+V** — the image lands in the focused box by itself, picking it was the whole gesture; token-gated). Upload decoding is Pillow-first with the HEIF opener registered — phone cameras default to **HEIC**, which OpenCV cannot read, and Pillow also applies the **EXIF orientation** (cv2.imdecode ignores it and portrait photos would paste rotated); OpenCV stays as the fallback decoder, and an undecodable upload logs its magic bytes.
