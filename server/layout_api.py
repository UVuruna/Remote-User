...
async def zoom_region(ws, layouts, conn: dict, msg: dict) -> None:
    ...
    rect = layout_zoom._norm(msg)
    zoom = None if layout_zoom._is_full(rect) else rect
    drawn = layout_zoom._drawn_of(msg)
    ...
    if (layout_zoom._rect_delta(zoom, conn.get("zoom")) < layout_zoom.ZOOM_MIN_DELTA
            and not layout_zoom._drawn_moved(drawn, conn.get("zoom_drawn"))):
        return
    conn["zoom"] = zoom
    conn["zoom_drawn"] = drawn
    if layout_zoom.zoom_step(conn) == conn.get("stream_zoom", 1):
        return
...
async def send_layout_state(ws, layouts, conn: dict, resuming=None) -> None:
    ...
    if conn.get("reset_stream") and (
            layout_zoom.stream_crop(conn) != conn.get("stream_region")
            or layout_zoom.zoom_step(conn) != conn.get("stream_zoom", 1)):
        conn["reset_stream"]()
