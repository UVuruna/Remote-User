...
- `snapshot() -> DisplaySnapshot` — the current truth, read now, never remembered (project `CLAUDE.md` constraint 13). Safe from any thread; `EnumDisplayMonitors` is synchronous and reentrant. Never raises — a failed enumeration is logged and answered with an empty tuple.
