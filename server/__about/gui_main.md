...
### Used by
- The installed EXE (`VibeCoder.exe`) — PyInstaller entry point, see [Setup (folder)](../../setup/___setup.md)
...
- `_claim_gui_singleton()`: the single-instance claim (see Purpose). `True` = this process owns `VibeCoderGuiSingleton` and may run; the handle is held for the whole process and never closed by us — the OS's release-on-death is exactly the lifetime a single-instance claim must have
