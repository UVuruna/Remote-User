...
- `_send_config(ws, stream, token, codec=None)`: builds and sends the `config` payload (monitor size, `stream` mode, `tailscale_url`, `app_version`, `base`, and `codec` when given)
