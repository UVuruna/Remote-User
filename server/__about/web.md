...
`GET /history` (registered by [Work History](work_history.md), 2026-08-21) — the desktop-only cross-project task history page, opened from the tray's "Work history" action. Its own responsibility and its own gate: loopback only (`127.0.0.1`/`::1`, no token — the phone never sees this route, and a LAN/WAN request gets a 403), registered the same way `upload_api` is, one line beside it.
